...
class TestProxyPool:
    ...
    async def test_pool_disabled_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr(settings.proxy_pool, "enabled", False)
    ...
    async def test_round_robin_picks_oldest(self, mock_db_session, monkeypatch):
        ...
        monkeypatch.setattr(settings.proxy_pool, "enabled", True)
        monkeypatch.setattr(settings.proxy_pool, "rotation_strategy", "round_robin")
    ...
    async def test_no_proxy_platforms(self, monkeypatch):
        ...
        monkeypatch.setattr(settings.proxy_pool, "enabled", True)
        monkeypatch.setattr(settings.proxy_pool, "no_proxy_platforms", ["google.com"])
