...
class ProxyPool:
    ...
    async def get_proxy_for_url(self, url: str) -> Proxy | None:
        ...
        """
        if not settings.proxy_pool.enabled:
            return None

        domain = urlparse(url).netloc.lower()

        # Check exclusion list
        if any(platform in domain for platform in settings.proxy_pool.no_proxy_platforms):
            log.debug("proxy_skipped_excluded_platform", domain=domain)
            return None

        # Check if proxy is forced for this platform
        is_forced = any(platform in domain for platform in settings.proxy_pool.force_proxy_platforms)
    ...
    async def _select_proxy(self) -> Proxy | None:
        ...
        async with get_db() as session:
            ...
            strategy = settings.proxy_pool.rotation_strategy
    ...
    async def _health_check_loop(self) -> None:
        ...
        while True:
            ...
            await asyncio.sleep(settings.proxy_pool.health_check_interval_seconds)
        ...
        async with self._semaphore:
            ...
            try:
                async with httpx.AsyncClient(
                    ...
                    response = await client.get(settings.proxy_pool.health_check_url)
